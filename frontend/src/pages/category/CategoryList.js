...
const CategoryList = () => {
    ...
    useEffect(() => {
        getAllCategories("/api/category-list", { headers: httptoken(getToken("access_token"))}).then((res) => {
    ...
    }, [JSON.stringify(categories), httptoken, getToken, setToken, deletedCategory._id, navigate]);


    const handleDelete = async (event) => {
        deleteCategory(`/api/delete/category/${event.target.value}`, { headers: httptoken(getToken("access_token")) }).then((res) => {
