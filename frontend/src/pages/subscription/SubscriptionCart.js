...
const SubscriptionCart = ({ products, initAmount, httptoken, getToken, setToken, setDeletedProduct }) => {
    ...
    const handleCheckout = (event) => {
        ...
        multipleSubHandler("/api/sub/multiple/create-checkout-session", { cartItems: products }, { headers: httptoken(token) }).then((res) => {
    ...
    const handleDelete = async (event) => {
        const token = getToken("access_token");
        deleteCartItem(`/api/sub/removefromcart/${event.target.value}`, { headers: httptoken(token) }).then((res) => {
            console.log(res.data);
            ...
            <div className='container-fluid mt-3 p-3 vh-100'>
                ...
                <div className='row'>
                    <div className='col-lg-9'>
                        <h1 className='fw-light text-center pt-2 align-self-center'>Subscription Cart </h1>
                        ...
                        return (
                            <div key={i} className='col-lg-4 sect p-3 rounded shadow bg-white'>
                                ...
                                <div className='mb-1'>
                                    ...
                                    <Link className='fw-semibold cart-name p-0' to={`/product/${p._id}`}>{p.prodName}</Link>
