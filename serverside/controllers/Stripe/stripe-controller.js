...
router.get('/stripe/customer/portal', async (req, res) => {
    try {
...
router.get('/checkout/success/regular/:sessionId',
    async (req, res) => {
        try {
            const decodedToken = await verifyAccessToken(req.headers["authorization"]);
...
router.get('/checkout/success/sub/:sessionId',
    async (req, res) => {
        try {
            const decodedToken = await verifyAccessToken(req.headers["authorization"]);
...
router.get('/checkout/failure/regular/:sessionId', async (req, res) => {
    try {
        const decodedToken = await verifyAccessToken(req.headers["authorization"]);
...
router.get('/checkout/failure/sub/:sessionId', async (req, res) => {
    try {
        const decodedToken = await verifyAccessToken(req.headers["authorization"]);
